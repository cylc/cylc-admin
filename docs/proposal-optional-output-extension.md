...
3. Task / Output completion:

   i. A task's outputs are complete if the completion condition is satisfied.
      ...
      2. Or, if success is optional, then the outputs are complete if it fails.
      3. Or, if submission is optional, then the outputs are complete if it
         submit-fails.
      4. Or, if expiry is optional, then the outputs are complete if it
         expires.
   ...
   ii. When a task enters a final state it becomes either complete or incomplete.

      The "final" task states are:
      * ``succeeded``
      * ``failed``
      * ``submit-failed``
      * ``expired``

      When a task enters a final state, it will not be automatically
      (re-)submitted so, in the absence of external intervention, its outputs
      are finalised.

      A task in its final state is incomplete if its outputs are incomplete,
      it can be removed as complete otherwise.

      This is the same as the previous proposal and the current implementation
...
4. `:expire` cannot be required.
   ...
   * We currently allow `:submit-fail` to be required but this doesn't really
     make sense.

5. The completion condition should be a configurable expression. The completion
   condition can be set to tolerate cases not handled in the graph or express
   more complex completion criterion.

   The expression should be pure Python, evaluated in a restricted context
   which permits only `and` & `or` operators, but not:
...
6. Expiry is only optional if it is explicitly referenced in the graph (e.g.
   ...
   If the `expired` output is set for `a`, `b` and `c`, then all three
   will have the `expired` task status which is a final task status.
   Additionally:
