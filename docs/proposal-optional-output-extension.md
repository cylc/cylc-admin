...
6. Expiry is only optional if it is explicitly referenced in the graph (e.g.
   ...
   * `a` will have incomplete outputs and be retained in the task pool.
   * `b` will have complete outputs (permitted in graph) and be removed from
     the task pool.
   * `c` will have complete outputs (permitted in completion expression) and be
